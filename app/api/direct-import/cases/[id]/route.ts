 ...
 * IOR CASE DETAIL API
 *
 * GET /api/direct-import/cases/[id] - Get case with messages and thread token
 * PATCH /api/direct-import/cases/[id] - Update case (status, priority)
 */
...
export async function GET(
  ...
  try {
    ...
    console.error('[API] GET /api/direct-import/cases/[id] error:', error);
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    console.error('[API] PATCH /api/direct-import/cases/[id] error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
