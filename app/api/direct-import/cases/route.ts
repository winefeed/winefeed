 ...
 * IOR CASES API
 *
 * GET /api/direct-import/cases - List cases with filters
 *   Query params: page, pageSize, status, priority, producerId, q
 * POST /api/direct-import/cases - Create new case
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[API] GET /api/direct-import/cases error:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API] POST /api/direct-import/cases error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
