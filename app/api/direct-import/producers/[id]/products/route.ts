 ...
 *
 * GET /api/direct-import/producers/[id]/products - List products with pagination
 *   Query params: page, pageSize, q, status (active/inactive), sort (name/vintage/created_at)
 * POST /api/direct-import/producers/[id]/products - Create new product
 */
...
export async function GET(
  ...
  try {
    ...
    console.error('[API] GET /api/direct-import/producers/[id]/products error:', error);
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    console.error('[API] POST /api/direct-import/producers/[id]/products error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
