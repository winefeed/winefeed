 ...
 * IOR PRODUCERS API
 *
 * GET /api/direct-import/producers - List all producers
 * POST /api/direct-import/producers - Create new producer
 */
...
function transformProducer(p: Record<string, unknown>) {
  return {
    ...
    combiTag: p.combi_tag ?? p.combiTag ?? null,
  };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[API] GET /api/direct-import/producers error:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API] POST /api/direct-import/producers error:', error);
    return NextResponse.json(
      { error: 'Internal server error', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
