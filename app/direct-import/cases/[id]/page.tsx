...
export default function CaseDetailPage() {
    ...
    async function fetchCase() {
      try {
        const [caseResponse, templatesResponse] = await Promise.all([
          fetch(`/api/direct-import/cases/${caseId}`),
  ...
  const handleSendMessage = async () => {
    ...
    setSending(true);
    try {
      const response = await fetch(`/api/direct-import/cases/${caseId}/messages`, {
        method: 'POST',
  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    try {
      const response = await fetch(`/api/direct-import/cases/${caseId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
