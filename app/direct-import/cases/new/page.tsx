...
export default function NewCasePage() {
    ...
    async function fetchData() {
      try {
        const [producersRes, templatesRes] = await Promise.all([
          fetch('/api/direct-import/producers?pageSize=100'),
          fetch('/api/direct-import/templates'),
        ]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Create the case
      const caseRes = await fetch('/api/direct-import/cases', {
        method: 'POST',
      ...
      const messageRes = await fetch(`/api/direct-import/cases/${newCase.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
