...
export default function NewFeedbackPage() {
  ...
  const fromPath = searchParams.get('from') || '/direct-import';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/direct-import/feedback', {
        method: 'POST',
      ...
      router.push('/direct-import/feedback');
    } catch (err) {
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Hjälp oss förbättra direktimport
          </p>
        </div>
      </div>

      {/* Context info */}
      {fromPath !== '/direct-import' && (
