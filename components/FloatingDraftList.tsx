...
export function FloatingDraftList() {
  ...
  // Minimized state — mobile still shows bottom bar, desktop shows FAB
  if (!isVisible) {
    return (
      <>
        {/* Mobile: always show bottom bar even when "hidden" */}
        <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-primary text-primary-foreground shadow-[0_-4px_12px_rgba(0,0,0,0.15)] safe-area-bottom">
          <button
          ...
          >
            ...
            <span className="flex-1 text-left text-sm font-medium">
              {draftList.count} vin{draftList.count !== 1 ? 'er' : ''} · {formatPrice(totalValue)}
            </span>
            <ArrowRight className="h-4 w-4" />
          </button>
        </div>
        {/* Desktop: minimized FAB */}
        <button
          onClick={() => setIsVisible(true)}
          className="hidden md:block fixed bottom-6 right-6 z-50 p-3 bg-primary text-primary-foreground rounded-full shadow-lg hover:bg-primary/90 transition-all"
        >
          <ShoppingCart className="h-5 w-5" />
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-primary text-primary-foreground shadow-[0_-4px_12px_rgba(0,0,0,0.15)] safe-area-bottom">
        ...
        >
          <div className="relative">
            <ShoppingCart className="h-5 w-5" />
            <span className="absolute -top-2 -right-2 bg-white text-primary text-[10px] font-bold rounded-full h-4 w-4 flex items-center justify-center">
              {draftList.count}
            </span>
          </div>
          <div className="flex-1 text-left">
            <p className="text-sm font-medium">
              {draftList.count} vin{draftList.count !== 1 ? 'er' : ''} · {totalBottles} fl
            </p>
          </div>
